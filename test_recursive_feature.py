"""Recursive distillation: 0.25x student from 0.5x feature-distilled teacher."""
import sys
...
# Use the feature-distilled 0.5x model as the teacher
TEACHER_CHECKPOINT = Path("runs/distill_0.5x_feature/checkpoints")
OUTPUT_DIR = Path("runs/recursive_0.25x_from_feature")
STUDENT_SCALE = 0.5  # 0.5 of 0.5x teacher = 0.25x of original

print("="*80)
print("RECURSIVE Distillation: 0.25x from Feature-Distilled 0.5x Teacher")
print("="*80)

# Load teacher (the 0.5x feature-distilled model)
print("\n1. Loading 0.5x teacher (feature-distilled)...")
...
distill_config = DistillationConfig(
    strategies=(DistillationStrategy.FEATURE,),  # Use same strategy as first level
    temperature=2.0,
    alpha=0.5,
    feature_weight=1.0,
)
...
print(f"   - Strategy: FEATURE (same as teacher's training)")
print(f"   - Epochs: {training_config.epochs}")
print(f"   - Learning rate: {training_config.learning_rate}")
print(f"   - Weight decay: {training_config.weight_decay}")

print(f"\n4. Starting recursive distillation...")
print(f"   Teacher chain: Original 128d → 64d (feature) → 32d (feature)")
print("="*80)
